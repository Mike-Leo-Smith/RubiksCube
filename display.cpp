//
// Created by mango on 2016/11/14.
//
...
{
	static const double sticker_size = 0.83;
	static const double block_size = 0.962;
	static const double pivot_size = 2.1;
	
	static Animation ani = Animation(N);
	
	static float rot_x = 20;
	static float rot_y = -30;
	static float rot_z = 0;
	
	static GLFWwindow *win;
	static const ROTATE_METHOD *operation;
	static int op_count = 0;
	static RubiksCube::Cube *cube;
	
	static void reshape(GLFWwindow *win, int w, int h);
	static void procKey(void);
	static void drawBlock(const COLOR_NAME face[], int x, int y, int z);
	static void drawPivot(void);
